...
BNB_DECLARE_SAMPLER_VIDEO(16, 17, glfx_VIDEO);
...
BNB_DECLARE_SAMPLER_2D(12, 13, tex_metallic);

BNB_DECLARE_SAMPLER_2D(14, 15, tex_roughness);
#ifdef GLFX_AO
#endif
#endif
#ifdef GLFX_TEX_EMI
...
void main()
{
    ...
    vec2 uv = var_uv;
    uv.x *= 0.5;
    vec3 rgb = BNB_TEXTURE_2D(BNB_SAMPLER_2D(glfx_VIDEO), uv).rgb;
    uv.x += 0.5;
